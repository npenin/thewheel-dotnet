...
namespace TheWheel.ServiceBus
{
        ...
        where TMessage : MessageBase
    {
        ...
        internal IEnumerable<TMessage> GetMessages()
        {
            ...
            EnsureConnectionIsOpen();
            var cmd = connection.CreateCommand();
            cmd.CommandText = "RECEIVE CONVERT(NVARCHAR(MAX), message_body), conversation_handle, message_type_name, conversation_group_id FROM [" + Queue + "]";
        ...
        private TMessage GetMessage()
        {
            ...
            EnsureConnectionIsOpen();
            var cmd = connection.CreateCommand();
            cmd.CommandText = "WAITFOR (RECEIVE TOP (1) CONVERT(NVARCHAR(MAX), message_body), conversation_handle, message_type_name, conversation_group_id FROM [" + Queue + "])";
            var timeout = Timeout;
