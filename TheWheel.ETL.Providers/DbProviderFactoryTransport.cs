...
namespace TheWheel.ETL.Providers
{
    public class DbProviderFactoryTransport : ITransport<IDbCommand>
    {
        ...
        public virtual async Task<IDbCommand> GetStreamAsync()
        {
            await Task.Run(command.Connection.Open);
        ...
        public Task<IDbCommand> QueryAsyncInternal(string query, params KeyValuePair<string, object>[] parameters)
        {
            return QueryAsyncInternal(new DbQuery(query, parameters), System.Threading.CancellationToken.None);
        }
